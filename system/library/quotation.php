...
class Quotation {
	...
	public function __construct($registry) {
		...
		if (!isset($this->session->data['quotation']) || !is_array($this->session->data['quotation'])) {
			$this->session->data['quotation'] = array();
		}
	}

	public function getProducts() {
		if (!$this->data) {
			foreach ($this->session->data['quotation'] as $key => $quantity) {
				...
				if (isset($product['qprice'])) {
					$qprice = $product['qprice'];
				} else {
					$qprice = 0;
				}

				if (isset($product['ogprice'])) {
				...
				if ($product_query->num_rows) {
					...
					if(!isset($this->session->data['adminpriceedit'])) {
						$price = $product_query->row['price'];

						// Product Discounts
						$discount_quantity = 0;

						foreach ($this->session->data['quotation'] as $key_2 => $quantity_2) {
	...
	public function getQProducts($quotation_id) {
			foreach ($this->session->data['cart'] as $key => $quantity) {
				...
				if ($product_query->num_rows) {
					...
					$product_price = $this->db->query("SELECT qprice FROM " . DB_PREFIX . "quotation_product  WHERE product_id = '" . (int)$product_id . "' AND quotation_id = '" . (int)$quotation_id . "' ORDER BY quotation_id DESC LIMIT 1");
					
					if($product_price->num_rows){
						$price = $product_price->row['qprice']; 
					} else {
						$price = 0;
					}
	...
	public function add($product_id, $qty = 1, $option = array(), $recurring_id = 0,$price_prefix="+",$percent="",$qprice=0,$price =0) {
		$this->data = array();
		...
		if (isset($this->session->data['adminpriceedit'])) {
			$product['price_prefix'] = $price_prefix;
			$product['percent'] = $percent;
			$product['qprice'] = $qprice;
			$product['ogprice'] = $price;
		}
		...
		if ((int)$qty && ((int)$qty > 0)) {
			if (!isset($this->session->data['quotation'][$key])) {
				$this->session->data['quotation'][$key] = (int)$qty;
			} else {
				$this->session->data['quotation'][$key] += (int)$qty;
			}
		}
	...
	public function addProduct($product_id, $qty = 1, $option = array(), $recurring_id = 0, $qprice=0) {
		$this->data = array();
	...
	public function update($key, $qty) {
		...
		if ((int)$qty && ((int)$qty > 0) && isset($this->session->data['quotation'][$key])) {
			$this->session->data['quotation'][$key] = (int)$qty;
		} else {
			$this->remove($key);
		}
	...
	public function remove($key) {
		...
		unset($this->session->data['quotation'][$key]);
	}
	...
	public function hasProducts() {
		return count($this->session->data['quotation']);
	}
