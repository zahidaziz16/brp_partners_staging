...
class Theme_ProductFieldSystemWidget extends Theme_SystemWidget
{
    public function render(array $view_data = array())
    {

        $field_id = (int) substr($this->getSlotName(), strlen('product_field_'));

        if (empty($field_id)) {
            return parent::render($view_data);
        }

        /** @var ProductFields_Catalog_DefaultModel $defaultModel */
        $defaultModel = $this->extension->getModel('default');

        $content = $defaultModel->getProductFieldContent($this->themeData->product_id, $field_id);

        if (!empty($content)) {
            $this->themeData->viewSlot->setContent($this->getSlotFullName(), $content);
        }
