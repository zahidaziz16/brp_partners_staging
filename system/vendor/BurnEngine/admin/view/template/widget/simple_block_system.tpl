<div id="basic_widget_content" class="s_widget_options_holder tb_cp">

  <style id="basic_widget_content_styles">
  #basic_widget_content .ui-tabs-panel [data-language_code]:not([data-language_code="<?php echo $tbData->first_language_code; ?>"]) {
    display: none;
  }
  </style>
        ...
        <div<?php if (count($languages) > 1) echo ' class="tb_tabs tb_fly_tabs tbLanguageTabs"'; ?>>
          ...
          <ul class="tb_tabs_nav clearfix">
            <?php foreach ($languages as $language): ?>
              <li class="s_language">
                <a href="#widget_basic_language_<?php echo $language['code']; ?>" title="<?php echo $language['name']; ?>">
          ...
          <?php foreach ($languages as $language): ?>
          <div id="widget_basic_language_<?php echo $language['code']; ?>" class="s_language_<?php echo $language['code']; ?>" data-language_code="<?php echo $language['code']; ?>"></div>
          <?php endforeach; ?>
  ...
  $(document).ready(function () {
    ...
    $("#widget_settings_holder").find(".tbLanguageTabs").first().tabs({
      activate: function (event, ui) {
        $("#basic_widget_content_styles").html('#basic_widget_content .ui-tabs-panel [data-language_code]:not([data-language_code="' + ui.newPanel.data("language_code") + '"]) { display: none; }');
      }
    });
