<div id="text_widget_content" class="s_widget_options_holder tb_cp">
  ...
  <h1 class="sm_title"><span>HTML Block</span></h1>
        ...
        <div<?php if (count($languages) > 1) echo ' class="tb_tabs tb_fly_tabs tbLanguageTabs"'; ?>>
          ...
          <div id="text_widget_language_<?php echo $language_code; ?>" class="s_language_<?php echo $language_code; ?>">
            ...
            <div class="s_row_2">
              <span class="s_language_icon"><img src="<?php echo $language['url'] . $language['image']; ?>" /></span>
              <div class="s_full">
                <textarea name="widget_data[lang][<?php echo $language_code; ?>][text]" id="text_widget_text_<?php echo $language_code; ?>" cols="43" rows="12"><?php echo htmlspecialchars($settings['lang'][$language_code]['text'], ENT_COMPAT, 'UTF-8'); ?></textarea>
              </div>
            </div>
          </div>
          <?php endforeach; ?>
...
$(document).ready(function() {
  ...
  <?php foreach ($languages as $language): ?>
  var cm<?php echo str_replace('-', '_', $language['code']); ?> = CodeMirror.fromTextArea(document.getElementById("text_widget_text_<?php echo $language['code']; ?>"), {
    mode:        "htmlmixed",
    lineNumbers: true,
    tabMode:     "indent"
  });
  ...
  $(tbApp).off("tbWidget:onUpdate.textWidget").one("tbWidget:onUpdate.textWidget", function(event, $widget, $form) {
    ...
    <?php foreach ($languages as $language): ?>
    cm<?php echo str_replace('-', '_', $language['code']); ?>.toTextArea();
    <?php endforeach; ?>
