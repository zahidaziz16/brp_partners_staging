<div id="product_attributes_system_widget_content" class="s_widget_options_holder tb_cp">

  <style id="product_attributes_system_widget_content_styles">
    #product_attributes_system_widget_content .ui-tabs-panel [data-language_code]:not([data-language_code="<?php echo $tbData->first_language_code; ?>"]) {
      display: none;
    }
  </style>

  <h1 class="sm_title"><span>Product Attributes</span></h1>
    ...
    <div class="tb_tabs tb_subpanel_tabs tbWidgetMainTabs">
      ...
      <div id="widget_settings_holder" class="tb_subpanel">
        <div<?php if (count($languages) > 1) echo ' class="tb_tabs tb_fly_tabs tbLanguageTabs"'; ?>>
          ...
          <?php if (count($languages) > 1): ?>
            <ul class="tb_tabs_nav clearfix">
              <?php foreach ($languages as $language): ?>
                <li class="s_language">
                  <a href="#widget_product_attributes_system_language_<?php echo $language['code']; ?>" title="<?php echo $language['name']; ?>">
          ...
          <?php foreach ($languages as $language): ?>
            <div id="widget_product_attributes_system_language_<?php echo $language['code']; ?>" class="s_language_<?php echo $language['code']; ?>" data-language_code="<?php echo $language['code']; ?>"></div>
          <?php endforeach; ?>
  ...
  $(document).ready(function () {
    ...
    $("#widget_settings_holder").find(".tbLanguageTabs").first().tabs({
      activate: function (event, ui) {
        $("#product_attributes_system_widget_content_styles").html('#product_attributes_system_widget_content .ui-tabs-panel [data-language_code]:not([data-language_code="' + ui.newPanel.data("language_code") + '"]) { display: none; }');
      }
    });
