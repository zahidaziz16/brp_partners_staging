...
class Theme_HtmlWidget extends TB_Widget
{
    ...
    public function onFilter(array &$settings)
    {
        ...
        if (isset($_POST['widget_data'])) {
            ...
            if (is_string($_POST['widget_data'])) {
                $widget_data = json_decode(html_entity_decode((string) $widget_data, ENT_COMPAT, 'UTF-8'), true);
            }
            foreach (array_keys($this->engine->getEnabledLanguages()) as $language_code) {
                if (isset($widget_data['lang'][$language_code]['text'])) {
                    $settings['lang'][$language_code]['text'] = base64_decode($widget_data['lang'][$language_code]['text']);
                }
            }
        }
    }
