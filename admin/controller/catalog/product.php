...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                        ...
                        if($this->model_catalog_product->getTotal3rdPartyProducts()>$this->config->get('config_max_3rd_party_product')) {
                                $this->response->setOutput($this->load->view('catalog/product_form_limit', $this->request->post));
                                $this->response->redirect($this->url->link('catalog/product/add', 'token=' . $this->session->data['token'], true));
                        } else {
	...
	protected function getList() {
		...
		//echo $this->request->get['filter_product_type'];
		...
		foreach ($results as $result) {
			...
			$result['model'] = html_entity_decode($result['model']);
            $result['matching_code'] = html_entity_decode($result['matching_code']);
			...
			$intNewERPBalance = "-";
			$intNewWMSBalance = "-";
                        $intNewQuantity = (isset($arrAPIResults["oc_data"][$result['product_id']])?$arrAPIResults["oc_data"][$result['product_id']]:"0");
			if(!$this->config->get('config_using_warehouse_module')&&$result['data_source']!=""&&$result['data_source']!="0") {
				$intNewERPBalance = (isset($arrAPIResults["erp_data"][$result['matching_code']])?str_replace(".0000","",$arrAPIResults["erp_data"][$result['matching_code']]):"0");
			} if($this->config->get('config_using_warehouse_module')&&$result['data_source']!=""&&$result['data_source']!="0") {
				$intNewERPBalance = (isset($arrAPIResults["erp_data"][$result['matching_code']])?str_replace(".0000","",$arrAPIResults["erp_data"][$result['matching_code']]):"0");
				$intNewWMSBalance = (isset($arrAPIResults["wms_data"][$result['matching_code']])?str_replace(".0000","",$arrAPIResults["wms_data"][$result['matching_code']]):"0");
			} if($this->config->get('config_using_warehouse_module')&&$result['data_source']=="") {
				//$intNewQuantity = (isset($arrAPIResults["oc_data"][$result['product_id']])?$arrAPIResults["oc_data"][$result['product_id']]:"0");
				$intNewWMSBalance = (isset($arrAPIResults["wms_data"][$result['model']])?str_replace(".0000","",$arrAPIResults["wms_data"][$result['model']]):"0");
			} if(!$this->config->get('config_using_warehouse_module')&&$result['data_source']=="") {
				//$intNewQuantity = (isset($arrAPIResults["oc_data"][$result['product_id']])?$arrAPIResults["oc_data"][$result['product_id']]:"0");
			}
			$data['products'][] = array(
		...
		$data['entry_image'] = $this->language->get('entry_image');
                $data['entry_brp'] = $this->language->get('entry_brp');
                $data['entry_3rd_party'] = $this->language->get('entry_3rd_party');
		...
		$data['sort_status'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, true);
                $data['sort_product_type'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.data_source' . $url, true);
	...
	protected function getForm($form_type = "add") {
		$data['heading_title'] = $this->language->get('heading_title');
		...
		if (isset($this->request->post['shipping'])) {
			...
			$data['shipping'] = 1;
                ...
                if (isset($this->request->post['force_update_price'])) {
			$data['force_update_price'] = $this->request->post['force_update_price'];
                } elseif (!empty($product_info)) {
			$data['force_update_price'] = $product_info['force_update_price'];
		} else {
			$data['force_update_price'] = 1;
		}
                
                if (isset($this->request->post['contain_serialno'])) {
			$data['contain_serialno'] = $this->request->post['contain_serialno'];
		} elseif (!empty($product_info)) {
			$data['contain_serialno'] = $product_info['contain_serialno'];
		} else {
			$data['contain_serialno'] = 0;
		}

		//Marketplace - Lazada & 11street
		if (isset($this->request->post['sync_lazada'])) {
			$data['sync_lazada'] = $this->request->post['sync_lazada'];
		} elseif (!empty($product_info)) {
			$data['sync_lazada'] = $product_info['sync_lazada'];
		} else {
			$data['sync_lazada'] = 1;
		}
		
		if (isset($this->request->post['sync_11street'])) {
			$data['sync_11street'] = $this->request->post['sync_11street'];
		} elseif (!empty($product_info)) {
			$data['sync_11street'] = $product_info['sync_11street'];
		} else {
			$data['sync_11street'] = 1;
		}
		...
		if (!empty($product_info)) { // new added for product ID
		...
		if (!empty($product_info)) { // new added for data source
			$data['data_source'] = $product_info['data_source'];
		} else {
			$data['data_source'] = "";
		}
		// Call to APIs - Start
		$apiParams = array(array("matching_code"=>$data['matching_code'], "product_id"=>$data['product_id'], "data_source"=>$data['data_source'], "model"=>$data['model']));
		$arrAPIResults = $this->ajaxAPI('stock_levels', $apiParams);
        ...
        $data['matching_code'] = html_entity_decode($data['matching_code']);

		if(!$this->config->get('config_using_warehouse_module') && ($data['data_source']!="" && $data['data_source']!="0")) {
			$data['erp_data'] = (isset($arrAPIResults["erp_data"][$data['matching_code']])&&$arrAPIResults["erp_data"][$data['matching_code']]!=".0000"?str_replace(".0000","",$arrAPIResults["erp_data"][$data['matching_code']]):"0");
		} if($this->config->get('config_using_warehouse_module') && ($data['data_source']!="" && $data['data_source']!="0")) {
			$data['erp_data'] = (isset($arrAPIResults["erp_data"][$data['matching_code']])&&$arrAPIResults["erp_data"][$data['matching_code']]!=".0000"?str_replace(".0000","",$arrAPIResults["erp_data"][$data['matching_code']]):"0");
			$data['wms_data'] = (isset($arrAPIResults["wms_data"][$data['matching_code']])?str_replace(".0000","",$arrAPIResults["wms_data"][$data['matching_code']]):""); // 0 Awaiting API
		} if($this->config->get('config_using_warehouse_module') && ($data['data_source']=="")) {
			$data['oc_data'] = (isset($arrAPIResults["oc_data"][$data['product_id']])?$arrAPIResults["oc_data"][$data['product_id']]:"0");
			$data['wms_data'] = (isset($arrAPIResults["wms_data"][$data['matching_code']])?str_replace(".0000","",$arrAPIResults["wms_data"][$data['matching_code']]):""); // 0 Awaiting API
		} if(!$this->config->get('config_using_warehouse_module') && ($data['data_source']=="")) {
			$data['oc_data'] = (isset($arrAPIResults["oc_data"][$data['product_id']])?$arrAPIResults["oc_data"][$data['product_id']]:"0");
		}
		...
		// Image
		$data['image_souce'] = "self";
		if (isset($this->request->post['image'])) {
			$data['image'] = $this->request->post['image'];
		} elseif (!empty($product_info) && isset($product_info['image']) && is_file(DIR_IMAGE . $product_info['image'])) {
			$data['image'] = $product_info['image'];
		} elseif (!empty($product_info) && isset($product_info['image']) && $product_info['image']!="") {
			$data['image_souce'] = "atoz";
			$data['image'] = $product_info['image'];
		} else {
			$data['image'] = '';
		}
		//echo "<pre>";print_r($product_info);echo "</pre>";exit;
		$data['data_souce'] = (isset($product_info['data_source'])&&$product_info['data_source']!=""?"atoz":"");
		...
		} elseif (!empty($product_info) && !empty($product_info['image'])) { // new added
		...
		foreach ($product_images as $product_image) {
			...
			} else if (!empty($product_image['image'])) { // new added
				$atoz_img = true;
				$image = $product_image['image'];
				$thumb = $product_image['image'];
			} else {
			...
			$data['product_images'][] = array(
				'atoz_img'   => $atoz_img, // new added
				'image'      => $image,
				'thumb'      => (!$atoz_img?$this->model_tool_image->resize($thumb, 100, 100):$thumb), // new added
		...
		if($form_type=="add" && ($this->model_catalog_product->getTotal3rdPartyProducts()>$this->config->get('config_max_3rd_party_product'))) {
			$this->response->setOutput($this->load->view('catalog/product_form_limit', $data));
		} else {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_name_model'])) {
			$this->load->model('catalog/product');
			$this->load->model('catalog/option');
			...
			if (isset($this->request->get['filter_name_model'])) {
				$filter_name_model = $this->request->get['filter_name_model'];
			} else {
				$filter_name_model = '';
			}
			...
			$filter_data = array(
				'filter_name'  => $filter_name,
				'filter_model' => $filter_model,
				'filter_name_model' => $filter_name_model,
				'start'        => 0,
				'limit'        => $limit
			);
