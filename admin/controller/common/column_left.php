			...
			if ($this->user->hasPermission('access', 'catalog/product')) {
				$catalog[] = array(
					...
					'children' => array()		
                        ...
                        if ($this->user->hasPermission('access', 'extension/module/bulk_product_editing')) {
				$catalog[] = array(
					'name'	   => "Bulk Product Editing",
					'href'     => $this->url->link('extension/module/bulk_product_editing', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
...
//			}
			...
			*/	
			...
			if ($this->user->hasPermission('access', 'sale/quotation')) {
				$sale[] = array(
					'name'	   => 'Quotation Manager',
					'href'     => $this->url->link('sale/quotation', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);	
			}
			...
			// Warehouse Transaction - New added
			$warehouse = array();
			if ($this->config->get('config_using_warehouse_module') && $this->user->hasPermission('access', 'warehouse/transactions')) {
				$warehouse[] = array(
					'name'	   => $this->language->get('text_transactions'),
					'href'     => $this->url->link('warehouse/transactions', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			
			// Upload Delivery List - New added
			if ($this->config->get('config_using_warehouse_module') && $this->user->hasPermission('access', 'warehouse/transactions')) {
				if ($this->user->hasPermission('access', 'sale/upload_delivery_list')) {
					$warehouse[] = array(
						'name'	   => $this->language->get('text_upload_delivery_list'),
						'href'     => $this->url->link('sale/upload_delivery_list', 'token=' . $this->session->data['token'], true),
						'children' => array()		
					);
				}
			}
			...
			$marketplace = array();
			$lazada = array();
			if ($this->config->get('config_lazada_sync') && $this->user->hasPermission('access', 'marketplace/lazada')) {
				$lazada[] = array(
					'name'	   => $this->language->get('text_setting'),
					'href'     => $this->url->link('marketplace/lazada', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
				if ($this->config->get('lazada_enable')){
					$lazada[] = array(
						//'name'	   => $this->language->get('text_setting'),
						'name'	   => 'Categories',
						'href'     => $this->url->link('marketplace/lazadaCategory', 'token=' . $this->session->data['token'], true),
						'children' => array()		
					);	
				}
				$marketplace[] = array(
			...
			if ($this->config->get('config_11street_sync') && $this->user->hasPermission('access', 'marketplace/11street')) {
				$marketplace[] = array(
					'name'	   => $this->language->get('text_11street'),
					'href'     => $this->url->link('marketplace/11street', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			
			if ($this->config->get('config_shopee_sync') && $this->user->hasPermission('access', 'marketplace/shopee')) {
				$shopee[] = array(
					'name'	   => $this->language->get('text_setting'),
					'href'     => $this->url->link('marketplace/shopee', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
				if ($this->config->get('shopee_enable')){
					$shopee[] = array(
						//'name'	   => $this->language->get('text_setting'),
						'name'	   => 'Categories',
						'href'     => $this->url->link('marketplace/shopeeCategory', 'token=' . $this->session->data['token'], true),
						'children' => array()		
					);	
				}
				$marketplace[] = array(
			...
			if ($this->user->hasPermission('access', 'setting/setting')) {
				$settings[] = array(
					'name'	   => $this->language->get('text_store_settings'),
					'href'     => $this->url->link('setting/store', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
			if ($this->user->hasPermission('access', 'setting/running_no')) {
				$settings[] = array(
					'name'	   => $this->language->get('text_running_no'),
			...
			if ($this->user->hasPermission('access', 'tool/data_sync')) {
				$tool[] = array(
					'name'	   => $this->language->get('text_data_sync'),
					'href'     => $this->url->link('tool/data_sync', 'token=' . $this->session->data['token'], true),
					'children' => array()		
				);
			}
//                        
//                        if ($this->user->hasPermission('access', 'tool/maxis_esms')) {
//				$tool[] = array(
//					'name'	   => $this->language->get('text_maxis_esms'),
//					'href'     => $this->url->link('tool/maxis_esms', 'token=' . $this->session->data['token'], true),
//					'children' => array()		
//				);
//			}
//			
			...
			if ($this->user->hasPermission('access', 'report/sale_order')) {
				$report_sale[] = array(
					...
					'children' => array()
                        ...
                        if ($this->user->hasPermission('access', 'report/sale_quotation')) {
				$report_sale[] = array(
					'name'	   => $this->language->get('text_report_sale_quotation'),
					'href'     => $this->url->link('report/sale_quotation', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
                        
//                        
//                        if ($this->user->hasPermission('access', 'report/sale_order_branch')) {
//				$report_sale[] = array(
//					'name'	   => $this->language->get('text_report_sale_order_branch'),
//					'href'     => $this->url->link('report/sale_order_branch', 'token=' . $this->session->data['token'], true),
//					'children' => array()
//				);
//			}
//			
