...
class ControllerCustomerCustomerGroup extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_customer_level'] = $this->language->get('entry_customer_level');
		...
		$data['customer_groups'] = $this->model_customer_customer_group->getCustomerGroups();
                ...
                if (isset($this->request->post['parent_customer_group_id'])) {
			$data['parent_customer_group_id'] = $this->request->post['parent_customer_group_id'];
		} elseif (!empty($customer_group_info)) {
			$data['parent_customer_group_id'] = $customer_group_info['parent_customer_group_id'];
		} else {
			$data['parent_customer_group_id'] = 0;
		}
                
                $this->load->model('customer/customer_group');

		$data['customer_levels'] = $this->model_customer_customer_group->getCustomerLevels();
                
                if (isset($this->request->post['customer_level_id'])) {
			$data['customer_level_id'] = $this->request->post['customer_level_id'];
		} elseif (!empty($customer_group_info)) {
			$data['customer_level_id'] = $customer_group_info['customer_level_id'];
		} else {
			$data['customer_level_id'] = 0;
		}
                
                $data['enable_approval'] = $this->config->get('config_enable_approval');
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['customer_group_description'] as $language_id => $value) {
			if ((utf8_strlen($value['name']) < 3) || (utf8_strlen($value['name']) > 32)) {
                ...
                if(isset($this->request->get['customer_group_id'])) {
                    if(!$this->model_customer_customer_group->notChild($this->request->get['customer_group_id'], $this->request->post['parent_customer_group_id'])) {
                        $this->error['warning'] = "The parent set for this group is one of its own child, please select a different parent.";
                    }
                }
		return !$this->error;
	}
