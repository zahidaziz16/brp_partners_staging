...
<table class="table">
	...
	<tbody>
		...
		<?php 
			...
			$tmp_attribs = array(
				'manufacturers', 'model', 'sku', 'upc', 'ean', 'jan', 'isbn', 'mpn', 'location', 'length', 'width', 'height', 'weight', 'tags', 'discounts',
			);
		...
		<?php foreach( $tmp_attribs as $tmp_attrib ) { if( $tmp_attrib == 'tags' && empty( $tagsSupport ) ) continue; ?>
			<tr>
				...
				<td class="left">
					<?php echo $text_collapsed_by_default; ?>: <?php echo mf_render_btn_collapsed( $text_yes, $text_no, $text_pc, $text_mobile, $_baseName . '[' . $tmp_attrib . '][collapsed]', empty( $_baseValues[$tmp_attrib]['collapsed'] ) ? '0' : $_baseValues[$tmp_attrib]['collapsed'] ); ?>
					&nbsp;&nbsp;&nbsp;
