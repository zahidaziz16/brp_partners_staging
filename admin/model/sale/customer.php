	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			return array(
				...
				'custom_field'   => unserialize(base64_decode($address_query->row['custom_field']))
			);
		}
	}
...
/***IRENE - EDITED****/
	public function getTotalCreditTransactions($data = array()) {
		$sql = "SELECT a.customer_transaction_id FROM " . DB_PREFIX . "customer_transaction a 
				INNER JOIN " . DB_PREFIX . "order b ON (a.customer_id = b.customer_id)";

		$implode = array();
		if (!empty($data['filter_name'])) {
			
			$implode[] = " AND CONCAT(b.firstname, ' ', b.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}

		if (!empty($data['filter_email'])) {
			$implode[] = " AND b.email LIKE '" . $this->db->escape($data['filter_email']) . "%'";
		}

		if (!empty($data['filter_order_id'])) {
			$implode[] = " AND a.order_id = '" . (int)$data['filter_order_id'] . "'";
		}

		if (isset($data['filter_status']) && !is_null($data['filter_status'])) {
			$implode[] = " AND b.order_status_id = '" . (int)$data['filter_status'] . "'";
		}

		if (!empty($data['filter_date_added'])) {
			$implode[] = " AND DATE(b.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}

		$sql .= " WHERE a.order_id > 0 AND a.description LIKE 'Order ID%' AND ROUND(a.amount,2) < 0 ";
		if ($implode) {
			$sql .= implode(" ", $implode);
		}
	...
	public function getCreditTransactions($data = array()) {
		$sql = "SELECT ct.order_id, CONCAT(c.firstname, ' ', c.lastname) AS name, c.email, os.name AS status, ct.amount AS total, ct.date_added, ct.customer_id
				FROM " . DB_PREFIX . "customer_transaction ct INNER JOIN " . DB_PREFIX . "order c ON (ct.customer_id = c.customer_id)
				INNER JOIN " . DB_PREFIX . "order_status os ON (c.order_status_id = os.order_status_id)";
		
		$implode = array();
		if (!empty($data['filter_name'])) {
			$implode[] = " AND CONCAT(c.firstname, ' ', c.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
		if (!empty($data['filter_email'])) {
			$implode[] = " AND c.email LIKE '" . $this->db->escape($data['filter_email']) . "%'";
		}
		if (!empty($data['filter_order_id'])) {
			$implode[] = " AND ct.order_id = '" . (int)$data['filter_order_id'] . "'";
		}
		if (isset($data['filter_status']) && !is_null($data['filter_status'])) {
			$implode[] = " AND c.order_status_id = '" . (int)$data['filter_status'] . "'";
		}
		if (!empty($data['filter_date_added'])) {
			$implode[] = " AND DATE(ct.date_added) = DATE('" . $this->db->escape($data['filter_date_added']) . "')";
		}

		$sql .= " WHERE ct.order_id > 0 AND ct.description LIKE 'Order ID%' AND ROUND(ct.amount, 2) < 0 AND c.order_status_id NOT IN (7,8,9,10,11,12,16) ";
		if ($implode) {
			$sql .= implode(" ", $implode);
		}
		$sql .= " GROUP BY ct.order_id ";
