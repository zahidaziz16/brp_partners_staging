...
// Error Handler
function error_handler_for_data_sync($errno, $errstr, $errfile, $errline) {
	global $registry;
	...
	if (($errors != "Fatal Error") && isset($request->get['route']) && ($request->get['route']!='tool/data_sync/download'))  {
		if ($config->get('config_error_display')) {
			echo '<b>' . $errors . '</b>: ' . $errstr . ' in <b>' . $errfile . '</b> on line <b>' . $errline . '</b>';
		}
	} else {
		$session->data['data_sync_error'] = array( 'errstr'=>$errstr, 'errno'=>$errno, 'errfile'=>$errfile, 'errline'=>$errline );
		$token = $request->get['token'];
		$link = $url->link( 'tool/data_sync', 'token='.$token, 'SSL' );
		header('Status: ' . 302);
...
function fatal_error_shutdown_handler_for_data_sync()
{
	$last_error = error_get_last();
	if ($last_error['type'] === E_ERROR) {
		// fatal error
		error_handler_for_data_sync(E_ERROR, $last_error['message'], $last_error['file'], $last_error['line']);
	}
}


class ModelToolDataSync extends Model {
	...
	protected function validateOptionColumns( &$reader ) {
		...
		$ok = true;
		$data_sync_settings_use_option_id = $this->config->get('data_sync_settings_use_option_id');
		$data_sync_settings_use_option_value_id = $this->config->get('data_sync_settings_use_option_value_id');
		$language_id = $this->getDefaultLanguageId();
		...
		$options = array();
		foreach ($query->rows as $row) {
			if ($data_sync_settings_use_option_id) {
				$option_id = $row['option_id'];
				if (!isset($options[$option_id])) {
					$options[$option_id] = array();
				}
				if ($data_sync_settings_use_option_value_id) {
					$option_value_id = $row['option_value_id'];
	...
	protected function validateAttributeColumns( &$reader ) {
		...
		$ok = true;
		$data_sync_settings_use_attribute_group_id = $this->config->get('data_sync_settings_use_attribute_group_id');
		$data_sync_settings_use_attribute_id = $this->config->get('data_sync_settings_use_attribute_id');
		$language_id = $this->getDefaultLanguageId();
	...
	protected function validateFilterColumns( &$reader ) {
		...
		$ok = true;
		$data_sync_settings_use_filter_group_id = $this->config->get('data_sync_settings_use_filter_group_id');
		$data_sync_settings_use_filter_id = $this->config->get('data_sync_settings_use_filter_id');
		$language_id = $this->getDefaultLanguageId();
		...
		$filter_groups = array();
		foreach ($query->rows as $row) {
			if ($data_sync_settings_use_filter_group_id) {
				$filter_group_id = $row['filter_group_id'];
				if (!isset($filter_groups[$filter_group_id])) {
					$filter_groups[$filter_group_id] = array();
				}
				if ($data_sync_settings_use_filter_id) {
					$filter_id = $row['filter_id'];
	...
	public function upload( $filename, $incremental=false ) {
		...
		set_error_handler('error_handler_for_data_sync',E_ALL);
		register_shutdown_function('fatal_error_shutdown_handler_for_data_sync');

		try {
			$this->session->data['data_sync_nochange'] = 1;
			...
			// Memory Optimization
			if ($this->config->get( 'data_sync_settings_use_import_cache' )) {
			...
			$this->clearCache();
			$this->session->data['data_sync_nochange'] = 0;
			$available_product_ids = array();
			$available_category_ids = array();
			$available_customer_ids = array();
			...
			$errfile = $e->getFile();
			$errno = $e->getCode();
			$this->session->data['data_sync_error'] = array( 'errstr'=>$errstr, 'errno'=>$errno, 'errfile'=>$errfile, 'errline'=>$errline );
...
//		}
	...
	protected function getProductFiltersXnew( $min_id, $max_id ) {
	...
	protected function populateCustomersWorksheet( &$worksheet, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
		...
		$max_id = $customers[$len-1]['customer_id'];
	...
	public function download( $export_type, $offset=null, $rows=null, $min_id=null, $max_id=null) {
		...
		//$boolTemp = $this->db->query("INSERT INTO `" . DB_PREFIX . "banner` (`name`, `status`) VALUES ('ZZ', '1')");
		$boolTemp = $this->db->query("UPDATE `" . DB_PREFIX . "banner` SET `name`='XXnew' WHERE `banner_id`='9' ");
		//echo "<pre>";print_r($boolTemp);echo "</pre>";exit;
		
		
		//$aTemp = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product` WHERE `product_id` = '" . (int)$product_id . "' AND `var` = '" . $this->db->escape($var) . "'")->rows;
		$aTemp = $this->db->query("SELECT * FROM `" . DB_PREFIX . "product` WHERE 1=1")->rows;
		//echo "<pre>";print_r($aTemp);echo "</pre>";exit;
		//echo "<pre>";print_r(DB_PREFIX);echo "</pre>";exit;
		...
		chdir( DIR_SYSTEM.'PHPExcel' );
		require_once( 'Classes/PHPExcel.php' );
		PHPExcel_Cell::setValueBinder( new PHPExcel_Cell_DataSyncValueBinder() );
		chdir( $cwd );
		...
		// Memory Optimization
		if ($this->config->get( 'data_sync_settings_use_export_cache' )) {
			$cacheMethod = PHPExcel_CachedObjectStorageFactory::cache_to_phpTemp;
			$cacheSettings = array( 'memoryCacheSize'  => '16MB' );  
			PHPExcel_Settings::setCacheStorageMethod($cacheMethod, $cacheSettings);  
		}
	...
	public function getNotifications() {
		...
		$result = $this->curl_get_contents("http://www.mhccorp.com/index.php?route=information/message&type=tool_data_sync_3_5&language_code=$language_code");
		if (stripos($result,'<html') !== false) {
			return '';
		}
		$result = "Michael Testing OpenCart Ajax function ....";
		return $result;
	}
	...
	public function getDefaultTaxClassID() {
		//echo "SELECT * FROM " . DB_PREFIX . "tax_class ORDER BY `tax_class_id` DESC LIMIT 1 ";exit;
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_class ORDER BY `tax_class_id` DESC LIMIT 1 ");
		return $query->row;
	}
	
	public function getProductRelatedNew($table = "", $arrMappingData = array()) {
		//echo "<pre>";print_r($arrMappingData);echo "</pre>";
		$sql = "SELECT * FROM " . $table . " WHERE 1=1 ";
		if(count($arrMappingData)>0) {
			$sql .= " AND product_id IN ('".implode("','",$arrMappingData)."')";
		}
	...
	public function getSyncedHistoryNew($strCondition = "") {
		$sql = "SELECT * FROM oc_sync_history WHERE 1=1 ";
		if($strCondition!="") {
			$sql .= $strCondition;
		}
		//echo $sql;exit;
        ...
        public function getSyncHistoryDate($table = "") {
		$sql = "SELECT sync_date FROM oc_sync_history WHERE sync_table='".$table."' ";
		...
		$query = $this->db->query($sql);
                if(empty($query->rows)) {
                    $array = array();
                    $data = array();
                    $data['sync_date'] = '0000-00-00 00:00:00';
	...
	public function getProductMappingsNew($arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT `product_id`, `data_source` FROM " . DB_PREFIX . "product WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
		$query = $this->db->query($sql);
		return $query->rows;
        ...
        public function getCategoryMappingsNew($arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT `category_id`, `data_source` FROM " . DB_PREFIX . "category WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
        ...
        public function getManufacturerMappingsNew($arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT `manufacturer_id`, `data_source` FROM " . DB_PREFIX . "manufacturer WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
	...
	public function getCustomTableNew($table = "", $arrKeys = array(), $arrKeyValues = array()) {
		//echo "<pre>";print_r($arrKeys);echo "</pre>";
		//echo "<pre>";print_r($arrKeyValues);echo "</pre>";
		$strConditions = "";
		foreach($arrKeyValues as $kvKey => $kvValue) {
			$strConditions .= " AND (";
			$arrOuterConditions = array();
			foreach($kvValue as $innerKVKey => $innerKVVlue) {
				$arrInnerConditions = array();
				foreach($arrKeys as $kKey => $kValue) {
					$arrInnerConditions[] = "`".$kValue."`='".$arrKeyValues[$kValue][$innerKVKey]."'";
				}
		...
		//echo "<pre>";print_r($strConditions);echo "</pre>";
		//exit;
		$sql = "SELECT * FROM " . $table . " WHERE 1=1 ";
		if($strConditions!="") {
			$sql .= $strConditions;
	...
	public function getProductsNew($data = array(), $arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT p.* FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND p.data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
		...
		if (isset($data['filter_status']) && !is_null($data['filter_status'])) {
			$sql .= " AND p.status = '" . (int)$data['filter_status'] . "'";
                ...
                if (isset($data['filter_force_update_price']) && !is_null($data['filter_force_update_price'])) {
			$sql .= " AND p.force_update_price = '" . (int)$data['filter_force_update_price'] . "'";
		}
        ...
        public function getCategoriesNew($data = array(), $arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT c.* FROM " . DB_PREFIX . "category c LEFT JOIN " . DB_PREFIX . "category_description pd ON (c.category_id = pd.category_id) WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND c.data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
        ...
        public function getManufacturersNew($data = array(), $arrDataSourceIDs = array()) {
		//$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		$sql = "SELECT m.* FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "manufacturer_description md ON (m.manufacturer_id = md.manufacturer_id) WHERE 1=1 ";
		if(count($arrDataSourceIDs)>0) {
			$sql .= " AND m.data_source IN ('".implode("','",$arrDataSourceIDs)."')";
		}
		//echo $sql;echo "<br />";//exit;
	...
	public function getProductDescriptionsXnew($product_id) {
	...
	public function getProductCategoriesXnew($product_id) {
	...
	public function getProductSpecialsNew($product_id) {
	...
	public function getProductStoresXnew($product_id) {
	...
	public function getProductRelated($product_id) {
        ...
        public function getTempTableData($dataFilter, $strConditions="") {
		$output = array();
                
                
                $sql = "SELECT p.*, pd.name as name FROM temp_oc_product p LEFT JOIN temp_oc_product_description pd ON (p.product_id = pd.product_id) WHERE 1=1";
                ...
                if (isset($dataFilter['filter_name'])&&!empty($dataFilter['filter_name'])) {
                        $sql .= " AND pd.name LIKE '" . $dataFilter['filter_name'] . "%'";
                }
                if (isset($dataFilter['filter_model'])&&!empty($dataFilter['filter_model'])) {
                        $sql .= " AND p.model LIKE '" . $dataFilter['filter_model'] . "%'";
                }
                if (isset($dataFilter['filter_price']) && !is_null($dataFilter['filter_price'])) {
                        $sql .= " AND p.price LIKE '" . $dataFilter['filter_price'] . "%'";
                }
                if (isset($dataFilter['filter_quantity']) && !is_null($dataFilter['filter_quantity'])) {
                        $sql .= " AND p.quantity = '" . (int)$dataFilter['filter_quantity'] . "'";
                }
                if (isset($dataFilter['filter_status']) && !is_null($dataFilter['filter_status'])) {
                        $sql .= " AND p.status = '" . (int)$dataFilter['filter_status'] . "'";
                }
                if (isset($data['filter_force_update_price']) && !is_null($data['filter_force_update_price'])) {
			$sql .= " AND p.force_update_price = '" . (int)$data['filter_force_update_price'] . "'";
		}
                if (isset($dataFilter['filter_image']) && !is_null($dataFilter['filter_image'])) {
                        if ($dataFilter['filter_image'] == 1) {
                                $sql .= " AND (p.image IS NOT NULL AND p.image <> '' AND p.image <> 'no_image.png')";
                        } else {
                                $sql .= " AND (p.image IS NULL OR p.image = '' OR p.image = 'no_image.png')";
                        }
                ...
                if (isset($dataFilter['sort']) && in_array($dataFilter['sort'], $sort_data)) {
                        $sql .= " ORDER BY " . $dataFilter['sort'];
                } else {
                        $sql .= " ORDER BY pd.name";
                }
                if (isset($dataFilter['order']) && ($dataFilter['order'] == 'DESC')) {
                        $sql .= " DESC";
                } else {
                        $sql .= " ASC";
                }
                if (isset($dataFilter['start']) || isset($dataFilter['limit'])) {
                        ...
                        $sql .= " LIMIT " . (int)$dataFilter['start'] . "," . (int)$dataFilter['limit'];
                }
                
		$query = $this->db->query($sql);

                $output['tables']['oc_product'] = array();
		foreach ($query->rows as $result) {
                    array_push($output['tables']['oc_product'], $result);
		}

		return $output;
	}
        
        public function getTempTableTotal($dataFilter, $strConditions="") {
                
                $sql = "SELECT COUNT(DISTINCT p.product_id) AS total FROM temp_oc_product p LEFT JOIN temp_oc_product_description pd ON (p.product_id = pd.product_id) WHERE 1=1";
		...
		$query = $this->db->query($sql);
                ...
                $output["tables"]["oc_product"][0]["total"] = $query->row['total'];
                
		return $output;
	}
        
        public function clearTable($table) {
		return $query = $this->db->query("TRUNCATE `".$table."`");

		
	}

	public function getRecurringsXnew($product_id) {
	...
	public function getProductIDBySourceID($data_source) {
		$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE data_source = '" . (int)$data_source . "'");
		$intProductID = 0;
		foreach ($query->rows as $result) {
			$intProductID = $result['product_id'];
		}
		return $intProductID;
	}
        
        public function getOptionIDBySourceID($data_source) {
		$query = $this->db->query("SELECT option_id FROM " . DB_PREFIX . "option WHERE data_source = '" . (int)$data_source . "'");
		$intProductID = 0;
		foreach ($query->rows as $result) {
			$intProductID = $result['option_id'];
		}
		return $intProductID;
	}
        
        public function getOptionValueIDBySourceID($data_source) {
		$query = $this->db->query("SELECT option_value_id FROM " . DB_PREFIX . "option_value WHERE data_source = '" . (int)$data_source . "'");
		$intProductID = 0;
		foreach ($query->rows as $result) {
			$intProductID = $result['option_value_id'];
		}
		return $intProductID;
	}
        
        public function getProductOptionIDBySourceID($data_source) {
		$query = $this->db->query("SELECT product_option_id FROM " . DB_PREFIX . "product_option WHERE data_source = '" . (int)$data_source . "'");
		$intProductID = 0;
		foreach ($query->rows as $result) {
			$intProductID = $result['product_option_id'];
		}
		return $intProductID;
	}
        
        public function getCategoryIDBySourceID($data_source) {
		$query = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "category WHERE data_source = '" . (int)$data_source . "'");
		$intCategoryID = 0;
		foreach ($query->rows as $result) {
			$intCategoryID = $result['category_id'];
		}
		return $intCategoryID;
	}
        
	
	public function syncDataFromDataServerList($path_url, $key, $data_type='data', $table='', $filter_data = '', $conditions = '') {
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => array(
				...
				'data_type'=> $data_type,
				'table' => $table,
				'filters' => ($filter_data!=""&&is_array($filter_data)?json_encode($filter_data):""),
		...
		//echo $resp;
		//echo "<pre>xx";print_r(json_decode('{"host_key":"4qV1dxEK7v8cZ35U7IIOMUYkNiiXje96","tables":{"oc_product":[{"product_id":"1","model":"XER WC4250S","sku":"XER WC4250S","matching_code":"XER WC4250S","upc":"","ean":"","jan":"","isbn":"","mpn":"","location":"","quantity":"0","stock_status_id":"6","in_stock_status_id":"6","image":"catalog\/IT \/Fuji Xerox\/Toner\/Xerox-WC4250s-Black-Toner-Cartridge-25K-XER-WC4250S.jpg","manufacturer_id":"26","shipping":"1","price":"488.0000","points":"0","tax_class_id":"9","date_available":"0000-00-00","weight":"0.00000000","weight_class_id":"1","length":"0.00000000","width":"0.00000000","height":"0.00000000","length_class_id":"1","subtract":"1","minimum":"1","sort_order":"0","status":"1","viewed":"202","date_added":"2016-01-24 15:55:47","date_modified":"2016-08-18 15:43:40","new_pro":"0","erp_status":"1","maxquantity":"0"}]} }', true));echo "</pre>";exit;
		//echo "<pre>xx";print_r(json_decode('{"host_key":"4qV1dxEK7v8cZ35U7IIOMUYkNiiXje96","tables":{"oc_product":[{"product_id":"1","model":"XER WC4250S","sku":"XER WC4250S","matching_code":"XER WC4250S","upc":"","ean":"","jan":"","isbn":"","mpn":"","location":"","quantity":"0","stock_status_id":"6","in_stock_status_id":"6","image":"catalog\/IT \/Fuji Xerox\/Toner\/Xerox-WC4250s-Black-Toner-Cartridge-25K-XER-WC4250S.jpg","manufacturer_id":"26","shipping":"1","price":"488.0000","points":"0","tax_class_id":"9","date_available":"0000-00-00","weight":"0.00000000","weight_class_id":"1","length":"0.00000000","width":"0.00000000","height":"0.00000000","length_class_id":"1","subtract":"1","minimum":"1","sort_order":"0","status":"1","viewed":"202","date_added":"2016-01-24 15:55:47","date_modified":"2016-08-18 15:43:40","new_pro":"0","erp_status":"1","maxquantity":"0"}]}}', true));echo "</pre>";exit;
        ...
        public function syncDataFromDataServer($path_url, $key, $data_type='data', $table='', $filter_data = '', $conditions = '', $startTable = 0, $startData = 0, $startDataFrom = 0) {
		//error_reporting(E_ALL);
		//ini_set('display_errors','On');
	...
	function ManualInsertRecord($arrFields = array(), $tablename = '', $enablePrepData = true) {
		//try {
			// Pre checking for mysql data.
			if(isset($enablePrepData) && $enablePrepData) {
				$arrFields = $this->mysqlPrepValues($arrFields);
			}
			...
			if ($this->db->query($sql)) {
				//$this->Set('curr_insert_id', $this->db->insert_id());
				return true;
			} else {
				return false;
			}
		//} catch(Exception $e) { }
	} // End: function ManualInsertRecord
	
        function ManualInsertIgnoreRecord($arrFields = array(), $tablename = '', $enablePrepData = true) {
		//try {
			// Pre checking for mysql data.
			if(isset($enablePrepData) && $enablePrepData) {
				$arrFields = $this->mysqlPrepValues($arrFields);
			...
			$sql = 'INSERT IGNORE INTO ' . $tablename . '(' . $field_list . ') VALUES (' . $field_values . ')';
			//echo $sql;exit;
			if ($this->db->query($sql)) {
				//$this->Set('curr_insert_id', $this->db->insert_id());
				return true;
	...
	//=============================================================================================
	// Function   : ManualInsertRecord
	// Description: Insert records
	//=============================================================================================
	function ManualInsertRecordDebug($arrFields = array(), $tablename = '', $enablePrepData = true) {
		// Pre checking for mysql data.
		if(isset($enablePrepData) && $enablePrepData) {
			$arrFields = $this->mysqlPrepValues($arrFields);
		}
	...
	function ManualUpdateRecordByConditions($arrFields = array(), $tablename = 'users', $strConditions = '', $enablePrepData = true) {
		//try {
			$total_fields = count($arrFields);
	...
	function ManualDeleteJoinRecord($tablename = '', $join, $strCondition = " AND 1=0 ") {
		//try {
			if($tablename != "") {
				$sql = "DELETE t FROM " . $tablename . " " .$join. " WHERE 1=1 " . $strCondition;
				//echo $sql;exit;
				if ($this->db->query($sql)){
					return true;
				} else {
        ...
        function ManualDeleteRecord($tablename = '', $strCondition = " AND 1=0 ") {
		//try {
			if($tablename != "") {
	...
	function mysqlPrepValues($value) {
		if(get_magic_quotes_gpc()){ 
			//stripslashes
			return $this->stripslashesArray($value);
		} else { 
			//addslashes
			return $this->addslashesArray($value);
		}
	}
		...
		if (is_array($arr)) {
			...
			for ($x=0;$x<count($keys);$x++) {
				...
				if (!is_object($arr[$keys[$x]])) {
					...
					$arr[$keys[$x]] = $this->stripslashesArray($arr[$keys[$x]]);
				}
			}
		...
		if (is_array($arr)) {
			...
			for ($x=0;$x<count($keys);$x++) {
				...
				if (!is_object($arr[$keys[$x]])) {
					...
					$arr[$keys[$x]] = $this->addslashesArray($arr[$keys[$x]]);
				}
			}
			return $arr; //return the stripped array
		} else {
