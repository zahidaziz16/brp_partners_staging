...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '1', approved = '" . (isset($customer_group_info['approval'])?((int)!$customer_group_info['approval']):"0") . "', date_added = NOW()");
	...
	public function editCustomer($data) {
        ...
        public function isPetronasUser() {
            $petronasUser = false;
		
                
                $parents = $this->recursiveGetParent((int)$this->customer->getGroupId());
                
                if(in_array(11, $parents) || (int)$this->customer->getGroupId() == 11) {
                    $petronasUser = true;
                }
        ...
        public function custParents() {
            $parents = array();
                ...
                foreach($parents AS $keyP => $valP) {
                    if($valP != 8) {
                        $query = $this->db->query("SELECT customer_id, email FROM oc_customer WHERE customer_group_id = '".$valP."'");

                        if($query->num_rows) {
                            foreach($query->rows AS $key => $value) {
                                $arrVal[$value['customer_id']] = $value['email'];
                            }
                        }
                    }
                }
        ...
        public function recursiveGetParent($parentId) {
            $arrVal = array();
            $query = $this->db->query("SELECT parent_customer_group_id FROM oc_customer_group WHERE customer_group_id = '".$parentId."'");
            
            if($query->num_rows) {
                foreach($query->rows AS $key => $value) {
                    array_push($arrVal, $value['parent_customer_group_id']);
                    foreach($this->recursiveGetParent($value['parent_customer_group_id']) AS $innerKey => $innerVal) {
