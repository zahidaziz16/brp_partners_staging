	...
	public function add() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (!$json) {
				...
				// Custom save unique order id - Start
				$dataUpdate = array();
				$dataUpdate['unique_order_id'] = $this->config->get('config_unique_brp_partner_id')."@".$json['order_id'];
				$this->model_checkout_order->editOrderUniqueID($json['order_id'], $dataUpdate);
				// Custom save unique order id - End
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				...
				if (!$json) {
					...
					// Custom save unique order id - Start
					$dataUpdate = array();
					$dataUpdate['unique_order_id'] = $this->config->get('config_unique_brp_partner_id')."@".$order_id;
					$this->model_checkout_order->editOrderUniqueID($order_id, $dataUpdate);
					// Custom save unique order id - End
