				...
				if($num_rows>0) {
					...
					while($row2 = mysql_fetch_array($result2, MYSQL_ASSOC)) {	
						...
						if($customer_id!="") {
							...
							$recipient_orderdate .= "|" . date("Y-m-d H:i:s", strtotime($row1['date_added']));
...
function ajaxAPI($apitype = '', $params) {
	...
	if($apitype=="upload_receiving_list") {
		...
		if (isset($params['recipient_orderdate'])) { $recipient_orderdate = $params['recipient_orderdate']; }
